// stg_vehicle_alerts_search.js — Vehicle Alerts search modal logic
...
$(document).ready(function () {
  ...
  const table = $('#svaSearchTable').DataTable({
    ajax: { url: '/modules/settings/notifications/stg_vehicle_alerts/stg_vehicle_alerts_data.php?action=list', dataSrc: '' },
    columns: [
      { data: 'ref' },
      { data: 'alert_name' },
  ...
  $('#sva-search-name').on('keyup',      function () { table.column(1).search(this.value).draw(); });
  $('#sva-search-condition').on('keyup', function () { table.column(2).search(this.value).draw(); });
  $('#sva-search-status').on('keyup',    function () { table.column(8).search(this.value).draw(); });

  $('#svaSearchTable tbody').on('click', 'tr', function () {
    const data = table.row(this).data();
    if (!data) return;
    bootstrap.Modal.getInstance(document.getElementById('svaSearchModal')).hide();
    document.dispatchEvent(new CustomEvent('stg_vehicle_alerts-selected', { detail: data }));
  });

  $('#svaSearchModal').on('shown.bs.modal', function () {
