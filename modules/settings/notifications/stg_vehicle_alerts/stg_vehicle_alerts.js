// stg_vehicle_alerts.js — Vehicle Alerts rule matrix Vue app
  ...
  createApp({
    ...
    methods: {
      async fetchNextRef() {
        this.loading = true;
        try {
          const r = await fetch('/modules/settings/notifications/stg_vehicle_alerts/stg_vehicle_alerts_data.php?action=next_ref');
      ...
      onPrint() {
        ...
        window.open('/modules/settings/notifications/stg_vehicle_alerts/stg_vehicle_alerts_print.php?' + p.toString(), '_blank');
      },
      ...
      async onSave() {
        ...
        if (this.mode === 'edit') {
          const chk = await fetch(`/modules/settings/notifications/stg_vehicle_alerts/stg_vehicle_alerts_data.php?action=exists&ref=${encodeURIComponent(this.form.ref)}`);
        ...
        try {
          ...
          const r = await fetch(`/modules/settings/notifications/stg_vehicle_alerts/stg_vehicle_alerts_data.php?action=${action}`, {
            method:  'POST',
    ...
    mounted() {
      document.addEventListener('stg_vehicle_alerts-selected', (e) => {
        const d = e.detail;
        this.form = {
          ref:             d.ref             ?? '',
