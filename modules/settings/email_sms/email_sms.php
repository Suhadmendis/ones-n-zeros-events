<?php /* modules/settings/email_sms/email_sms.php — Email & SMS Settings editor */ ?>

<div id="ems-app" class="row g-4">
    ...
    <template v-else>
      <div v-for="(items, group) in grouped" :key="group" class="card card-outline card-secondary mb-4">
        <div class="card-header">
          <div class="card-title"><i class="bi bi-envelope-at me-2"></i>{{ group }}</div>
        </div>
        <div class="card-body">
          <div class="row">
            <div v-for="s in items" :key="s.setting_key" class="col-md-6 mb-4">
              ...
              <div v-if="s.setting_type === 'boolean'" class="form-check form-switch mt-1">
                <input class="form-check-input" type="checkbox" :id="'ems-' + s.setting_key"
                  :checked="s.setting_value === 'true'"
                  @change="update(s, $event.target.checked ? 'true' : 'false')" />
                <label class="form-check-label" :for="'ems-' + s.setting_key">
...
<script src="/modules/settings/email_sms/email_sms.js"></script>
