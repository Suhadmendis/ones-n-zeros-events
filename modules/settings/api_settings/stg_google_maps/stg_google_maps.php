<?php /* modules/settings/api_settings/stg_google_maps/stg_google_maps.php — Google Maps API Settings editor */ ?>

<div id="gmaps-app" class="row g-4">
    ...
    <template v-else>
      <div v-for="(items, group) in grouped" :key="group" class="card card-outline card-secondary mb-4">
        <div class="card-header">
          <div class="card-title"><i class="bi bi-geo-alt me-2"></i>{{ group }}</div>
        </div>
        <div class="card-body">
          <div class="row">
            <div v-for="s in items" :key="s.setting_key" class="col-md-6 mb-4">
              ...
              <div v-if="s.setting_type === 'boolean' || s.setting_type === 'toggle'" class="form-check form-switch mt-1">
                <input class="form-check-input" type="checkbox" :id="'gmaps-' + s.setting_key"
                  :checked="s.setting_value === 'true'"
                  @change="update(s, $event.target.checked ? 'true' : 'false')" />
                <label class="form-check-label" :for="'gmaps-' + s.setting_key">
...
<script src="/modules/settings/api_settings/stg_google_maps/stg_google_maps.js"></script>
