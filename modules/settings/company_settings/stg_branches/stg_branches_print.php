<?php
// modules/settings/company_settings/stg_branches/stg_branches_print.php — Branch print view
...
$branch_name    = htmlspecialchars($_GET['branch_name']    ?? '');
$branch_code    = htmlspecialchars($_GET['branch_code']    ?? '');
$address        = htmlspecialchars($_GET['address']        ?? '');
$city           = htmlspecialchars($_GET['city']           ?? '');
$phone          = htmlspecialchars($_GET['phone']          ?? '');
$email          = htmlspecialchars($_GET['email']           ?? '');
$manager_name   = htmlspecialchars($_GET['manager_name']   ?? '');
$is_head_office = ($_GET['is_head_office'] ?? '0') === '1';
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge-head     { color: #084298; background: #cfe2ff; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; }
...
<body>
  ...
  <div class="page">
    ...
    <table class="details">
      ...
      <tr><th>Branch Code</th><td><?= $branch_code ?></td></tr>
      <tr><th>Branch Name</th><td><?= $branch_name ?></td></tr>
      <?php if ($address): ?><tr><th>Address</th><td><?= nl2br($address) ?></td></tr><?php endif; ?>
      <?php if ($city): ?><tr><th>City</th><td><?= $city ?></td></tr><?php endif; ?>
      <?php if ($phone): ?><tr><th>Phone</th><td><?= $phone ?></td></tr><?php endif; ?>
      <?php if ($email): ?><tr><th>Email</th><td><?= $email ?></td></tr><?php endif; ?>
      <?php if ($manager_name): ?><tr><th>Manager Name</th><td><?= $manager_name ?></td></tr><?php endif; ?>
      <tr><th>Head Office</th><td><?= $is_head_office ? '<span class="badge-head">Yes</span>' : 'No' ?></td></tr>
