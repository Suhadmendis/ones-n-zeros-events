// attendance.js — generated by Generate Module
...
createApp({
  ...
  mounted() {
    this.fetchRefNumber();
    document.addEventListener('attendance-selected', (e) => this.loadRecord(e.detail));
    document.addEventListener('attn-employee-selected', (e) => {
  ...
  methods: {
    ...
    onPrint() {
      ...
      axios.get('/modules/human_resources_payroll/hr_operations/attendance/attendance_data.php?action=exists&ref=' + encodeURIComponent(this.form.ref))
        .then(res => {
          if (!res.data.exists) {
        ...
        attendance_date: String(this.form.attendance_date ?? ''),
        attendance_status: String(this.form.attendance_status ?? ''),
        remarks: String(this.form.remarks ?? ''),
          });
          window.open('/modules/human_resources_payroll/hr_operations/attendance/attendance_print.php?' + params.toString(), '_blank');
        })
    ...
    onSave() {
      ...
      const proceed = (action) => {
        axios.post('/modules/human_resources_payroll/hr_operations/attendance/attendance_data.php?action=' + action, this.form)
          .then(() => {
            this.saved = true;
            this.onReset();
            this.fetchRefNumber();
    ...
    loadRecord(data) {
      ...
      this.form.attendance_date = data.attendance_date;
      this.form.attendance_status = data.attendance_status;
