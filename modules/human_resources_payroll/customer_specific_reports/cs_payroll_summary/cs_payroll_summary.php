    ...
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="reportInfoModalLabel">
          <i class="bi bi-info-circle me-2 text-primary"></i>About: CS Payroll Summary
        </h5>
      ...
      <div class="modal-body">
        ...
        <p class="mb-3">
          <strong>CS Payroll Summary</strong> shows the totals and employee-level breakdown of a
          payroll run that has already been processed through <strong>Payroll Runs</strong>, for a
          selected Payroll Period. It answers: <em>"How much did this payroll period cost, and what
          did each employee get paid?"</em>
        </p>
...
<div id="cs-payroll-summary-app" v-cloak>
  <div class="card mb-3">
    <div class="card-body">
        ...
        <table class="table table-sm table-hover align-middle mb-0">
          <thead class="table-light">
            <tr>
              ...
              <th class="text-end">Budget Allowance 2025</th>
              <th class="text-end">BUDGET ALLOW/GOV.ALLOW 2016/2019</th>
              <th class="text-end">Transport Allowance</th>
              <th class="text-end">Gross Earnings</th>
              <th class="text-end">EPF Employee Contribution</th>
          ...
          <tbody>
            <tr v-for="(row, i) in summary.employees" :key="i">
              ...
              <td class="text-end">{{ fmt(row.budget_allowance_2025) }}</td>
              <td class="text-end">{{ fmt(row.budget_allow_gov_allow) }}</td>
              <td class="text-end">{{ fmt(row.transport_allowance) }}</td>
              <td class="text-end">{{ fmt(row.gross_earnings) }}</td>
              <td class="text-end text-danger">{{ fmt(row.epf_employee_contribution) }}</td>
            ...
            <tr v-if="!summary.employees.length">
              <td colspan="10" class="text-center text-muted py-3">No employees found in this payroll run.</td>
            </tr>
          </tbody>
        </table>
      </div>
...
<script src="/modules/human_resources_payroll/customer_specific_reports/cs_payroll_summary/cs_payroll_summary.js"></script>
