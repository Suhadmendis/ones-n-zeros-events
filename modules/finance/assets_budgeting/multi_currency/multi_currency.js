// multi_currency.js — Multi Currency Vue app
  ...
  createApp({
    ...
    methods: {
      async fetchNextRef() {
        this.loading = true;
        try {
          const r = await fetch('/modules/finance/assets_budgeting/multi_currency/multi_currency_data.php?action=next_ref');
      ...
      onPrint() {
        ...
        window.open('/modules/finance/assets_budgeting/multi_currency/multi_currency_print.php?' + p.toString(), '_blank');
      },
      ...
      async onSave() {
        ...
        if (this.mode === 'edit') {
          const chk = await fetch(`/modules/finance/assets_budgeting/multi_currency/multi_currency_data.php?action=exists&ref=${encodeURIComponent(this.form.ref)}`);
        ...
        try {
          ...
          const r = await fetch(`/modules/finance/assets_budgeting/multi_currency/multi_currency_data.php?action=${action}`, {
            method:  'POST',
    ...
    mounted() {
      document.addEventListener('multi_currency-selected', (e) => {
        const d = e.detail;
        this.form = {
          ref:              d.ref              ?? '',
