<?php
// modules/finance/budgeting/budgeting_print.php — Budgeting print view
...
$budget_name   = htmlspecialchars($_GET['budget_name']   ?? '');
$account_code  = htmlspecialchars($_GET['account_code']  ?? '');
$period        = htmlspecialchars($_GET['period']        ?? '');
$budget_type   = htmlspecialchars($_GET['budget_type']   ?? '');
$category      = htmlspecialchars($_GET['category']      ?? '');
$budget_amount = (float) ($_GET['budget_amount']         ?? 0);
$actual_amount = (float) ($_GET['actual_amount']         ?? 0);
$variance      = (float) ($_GET['variance']              ?? 0);
$notes         = htmlspecialchars($_GET['notes']         ?? '');
$status        = htmlspecialchars($_GET['status']        ?? '');

$status_labels = ['draft' => 'Draft', 'approved' => 'Approved', 'closed' => 'Closed'];
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .var-pos { color: #198754; font-weight: 700; }
...
<body>
  ...
  <div class="page">
    ...
    <table class="details">
      ...
      <tr><th>Budget Name</th><td><?= $budget_name ?></td></tr>
      <tr><th>Account Code</th><td><?= $account_code ?></td></tr>
      <tr><th>Period</th><td><?= $period ?></td></tr>
      <tr><th>Budget Type</th><td><?= $budget_type ?></td></tr>
      <tr><th>Category</th><td><?= $category ?></td></tr>
      <tr><th>Status</th><td><?= $status_label ?></td></tr>
      <?php if ($notes): ?><tr><th>Notes</th><td><?= nl2br($notes) ?></td></tr><?php endif; ?>
      <tr><th>Budget Amount</th><td class="num"><?= $fmt($budget_amount) ?></td></tr>
      <tr><th>Actual Amount</th><td class="num"><?= $fmt($actual_amount) ?></td></tr>
      <tr>
        <th>Variance</th>
        <td class="num <?= $variance >= 0 ? 'var-pos' : 'var-neg' ?>"><?= $fmt($variance) ?></td>
      </tr>
    </table>
