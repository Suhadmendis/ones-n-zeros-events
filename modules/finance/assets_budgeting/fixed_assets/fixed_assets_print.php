<?php
// modules/finance/fixed_assets/fixed_assets_print.php — Fixed Assets print view
...
$asset_name               = htmlspecialchars($_GET['asset_name']               ?? '');
$asset_category           = htmlspecialchars($_GET['asset_category']           ?? '');
$purchase_date            = htmlspecialchars($_GET['purchase_date']            ?? '');
$purchase_cost            = (float) ($_GET['purchase_cost']                    ?? 0);
$salvage_value            = (float) ($_GET['salvage_value']                    ?? 0);
$useful_life_years        = (int)   ($_GET['useful_life_years']                ?? 0);
$depreciation_method      = htmlspecialchars($_GET['depreciation_method']      ?? '');
$accumulated_depreciation = (float) ($_GET['accumulated_depreciation']         ?? 0);
$net_book_value           = (float) ($_GET['net_book_value']                   ?? 0);
$location                 = htmlspecialchars($_GET['location']                 ?? '');
$description              = htmlspecialchars($_GET['description']              ?? '');
$status                   = htmlspecialchars($_GET['status']                   ?? '');

$status_labels = ['active' => 'Active', 'disposed' => 'Disposed', 'fully_depreciated' => 'Fully Depreciated'];
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .nbv-row td { font-weight: 700; font-size: 13px; }
...
<body>
  ...
  <div class="page">
    ...
    <table class="details">
      ...
      <tr><th>Asset Name</th><td><?= $asset_name ?></td></tr>
      <tr><th>Category</th><td><?= $asset_category ?></td></tr>
      <tr><th>Location</th><td><?= $location ?: '—' ?></td></tr>
      <tr><th>Purchase Date</th><td><?= $purchase_date ?></td></tr>
      <tr><th>Useful Life</th><td><?= $useful_life_years ?> year(s)</td></tr>
      <tr><th>Depreciation Method</th><td><?= $depreciation_method ?></td></tr>
      <tr><th>Description</th><td><?= nl2br($description) ?: '—' ?></td></tr>
      <tr><th>Status</th><td><?= $status_label ?></td></tr>
      <tr><th>Purchase Cost</th><td class="num"><?= $fmt($purchase_cost) ?></td></tr>
      <tr><th>Salvage Value</th><td class="num"><?= $fmt($salvage_value) ?></td></tr>
      <tr><th>Accum. Depreciation</th><td class="num"><?= $fmt($accumulated_depreciation) ?></td></tr>
      <tr class="nbv-row"><th>Net Book Value</th><td class="num"><?= $fmt($net_book_value) ?></td></tr>
    </table>
