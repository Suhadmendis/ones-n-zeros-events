// accounts_receivable.js — Accounts Receivable Vue app
...
createApp({
  ...
  computed: {
    ...
    isDirty() {
      return this.form.customer_name.trim() !== '' || this.form.invoice_date !== '';
    },
    financeEnabled() {
  ...
  mounted() {
    ...
    axios.get('/server/module_visibility/module_visibility.php')
      ...
      .catch(() => {});
    document.addEventListener('accounts_receivable-selected', (e) => this.loadRecord(e.detail));
  },
  ...
  methods: {
    ...
    onPrint() {
      ...
      axios.get('/modules/finance/receivables_payables/accounts_receivable/accounts_receivable_data.php?action=exists&ref=' + encodeURIComponent(this.form.ref))
        .then(res => {
          if (!res.data.exists) { this.error = 'No saved record found. Please search and select a record to print.'; return; }
          const params = new URLSearchParams({
            ref:           this.form.ref,
            customer_name: this.form.customer_name,
            customer_ref:  this.form.customer_ref,
          ...
          window.open('/modules/finance/accounts_receivable/accounts_receivable_print.php?' + params.toString(), '_blank');
        })
    ...
    onSave() {
      ...
      const proceed = (action) => {
        axios.post('/modules/finance/receivables_payables/accounts_receivable/accounts_receivable_data.php?action=' + action, this.form)
          .then(() => {
            this.saved = true;
            this.onReset();
            this.fetchRefNumber();
