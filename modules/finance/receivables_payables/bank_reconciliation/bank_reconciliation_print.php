<?php
// modules/finance/bank_reconciliation/bank_reconciliation_print.php — Bank Reconciliation print view
...
$reconciliation_date  = htmlspecialchars($_GET['reconciliation_date']  ?? '');
$period               = htmlspecialchars($_GET['period']               ?? '');
$bank_balance         = (float) ($_GET['bank_balance']                 ?? 0);
$book_balance         = (float) ($_GET['book_balance']                 ?? 0);
$outstanding_deposits = (float) ($_GET['outstanding_deposits']         ?? 0);
$outstanding_cheques  = (float) ($_GET['outstanding_cheques']          ?? 0);
$adjusted_bank        = (float) ($_GET['adjusted_bank']                ?? 0);
$difference           = (float) ($_GET['difference']                   ?? 0);
$notes                = htmlspecialchars($_GET['notes']                ?? '');
$status               = htmlspecialchars($_GET['status']               ?? '');

$status_labels = ['draft' => 'Draft', 'reconciled' => 'Reconciled', 'locked' => 'Locked'];
...
<head>
  ...
  <title>Bank Reconciliation — <?= $ref ?></title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .details th { width: 52%; padding: 9px 14px; text-align: left; font-weight: 600; font-size: 12px; color: #333; border: 1px solid #ddd; background: #f0f0f0; }
    ...
    .section-head { font-size: 12px; font-weight: 700; text-transform: uppercase; color: #1a1a2e; padding: 8px 14px; background: #e8eaf6; border: 1px solid #c5cae9; }
    .diff-ok  { color: #198754; font-weight: 700; }
    .diff-err { color: #dc3545; font-weight: 700; }
    .badge-draft      { color: #41464b; background: #e2e3e5; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; }
    .badge-reconciled { color: #0a3622; background: #d1e7dd; padding: 2px 8px; border-radius: 4px; font-size: 11px; font-weight: 600; }
...
<body>
  ...
  <div class="page">
    ...
    <div class="doc-title">
      ...
      <div class="ref">Ref: <?= $ref ?> &mdash; Period: <?= $period ?></div>
    </div>

    <table class="details">
      <tr><th>Reference No.</th><td><?= $ref ?></td></tr>
      <tr><th>Account Name</th><td><?= $account_name ?></td></tr>
      <?php if ($account_number): ?><tr><th>Account Number</th><td><?= $account_number ?></td></tr><?php endif; ?>
      <tr><th>Period</th><td><?= $period ?></td></tr>
      <tr><th>Reconciliation Date</th><td><?= $reconciliation_date ?></td></tr>
    ...
    <table class="details">
      <tr><td colspan="2" class="section-head">Bank Statement Reconciliation</td></tr>
      <tr><th>Bank Statement Balance</th><td class="num"><?= $fmt($bank_balance) ?></td></tr>
      <tr><th>Add: Outstanding Deposits</th><td class="num"><?= $fmt($outstanding_deposits) ?></td></tr>
      <tr><th>Less: Outstanding Cheques</th><td class="num">(<?= $fmt($outstanding_cheques) ?>)</td></tr>
      <tr><th style="font-weight:700">Adjusted Bank Balance</th><td class="num" style="font-weight:700"><?= $fmt($adjusted_bank) ?></td></tr>
      <tr><td colspan="2" class="section-head">Book Balance Comparison</td></tr>
      <tr><th>Book Balance</th><td class="num"><?= $fmt($book_balance) ?></td></tr>
      <tr>
        <th style="font-weight:700">Difference</th>
        <td class="num <?= abs($difference) <= 0.01 ? 'diff-ok' : 'diff-err' ?>"><?= $fmt($difference) ?></td>
      </tr>
    </table>
