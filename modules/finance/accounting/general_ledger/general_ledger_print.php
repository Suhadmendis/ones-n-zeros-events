<?php
// modules/finance/general_ledger/general_ledger_print.php — GL print view
...
$transaction_date = htmlspecialchars($_GET['transaction_date'] ?? '');
$period           = htmlspecialchars($_GET['period']           ?? '');
$description      = htmlspecialchars($_GET['description']      ?? '');
$debit_amount     = htmlspecialchars($_GET['debit_amount']     ?? '');
$credit_amount    = htmlspecialchars($_GET['credit_amount']    ?? '');
$journal_ref      = htmlspecialchars($_GET['journal_ref']      ?? '');
$status           = htmlspecialchars($_GET['status']           ?? '');

$status_labels = ['draft' => 'Draft', 'posted' => 'Posted', 'reversed' => 'Reversed'];
$status_label  = $status_labels[$status] ?? $status;

$fmt = fn($v) => $v !== '' ? number_format((float)$v, 2) : '0.00';
?>
<!doctype html>
<html lang="en">
<head>
  ...
  <title>General Ledger — <?= $ref ?></title>
  <style>
...
<body>
  ...
  <div class="page">
    ...
    <table class="details">
      ...
      <tr><th>Debit Amount</th><td class="num"><?= $fmt($debit_amount) ?></td></tr>
      <tr><th>Credit Amount</th><td class="num"><?= $fmt($credit_amount) ?></td></tr>
      <tr><th>Journal Ref</th><td><?= $journal_ref ?: '—' ?></td></tr>
