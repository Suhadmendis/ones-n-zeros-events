<?php
// entries/cash_flow/cash_flow_print.php — Cash flow entry print view
...
$flow_type   = htmlspecialchars($_GET['flow_type']   ?? '');
$category    = htmlspecialchars($_GET['category']    ?? '');
$amount      = htmlspecialchars($_GET['amount']      ?? '');
$description = htmlspecialchars($_GET['description'] ?? '');

$flow_label = $flow_type === 'inflow' ? 'Inflow' : ($flow_type === 'outflow' ? 'Outflow' : $flow_type);
$flow_color = $flow_type === 'inflow' ? '#198754' : ($flow_type === 'outflow' ? '#dc3545' : '#6c757d');
?>
<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <title>Cash Flow Entry — <?= $ref ?></title>
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .badge-flow { display: inline-block; padding: 3px 10px; border-radius: 12px; font-size: 11px; font-weight: 600; color: #fff; }
...
<body>
  ...
  <div class="page">
    ...
    <div class="doc-title">
      <h2>Cash Flow Entry</h2>
      <div class="ref">Ref: <?= $ref ?></div>
    </div>
    <table class="details">
      <tr><th>Reference No.</th><td><?= $ref ?></td></tr>
      <tr><th>Date</th><td><?= $date ?></td></tr>
      <tr><th>Flow Type</th><td><span class="badge-flow" style="background:<?= $flow_color ?>"><?= $flow_label ?></span></td></tr>
      <tr><th>Category</th><td><?= $category ?></td></tr>
      <tr><th>Amount</th><td><strong>LKR <?= number_format((float)$amount, 2) ?></strong></td></tr>
      <?php if ($description !== ''): ?>
