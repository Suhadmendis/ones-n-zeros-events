<?php
// entries/fuel_entry/fuel_entry_print.php — Fuel entry print view
...
$liters          = htmlspecialchars($_GET['liters']          ?? '');
$rate            = htmlspecialchars($_GET['rate']            ?? '');
$total           = htmlspecialchars($_GET['total']           ?? '');
$voucher_number  = htmlspecialchars($_GET['voucher_number']  ?? '');
?>
<!doctype html>
<html lang="en">
<head>
  <meta charset="UTF-8" />
  <title>Fuel Entry — <?= $ref ?></title>
  <style>
...
<body>
  ...
  <div class="page">
    ...
    <table class="details">
      ...
      <tr><th>Liters</th><td><?= $liters ?> L</td></tr>
      <tr><th>Rate per Liter</th><td>LKR <?= $rate ?></td></tr>
      <?php if ($voucher_number): ?><tr><th>Voucher No.</th><td><?= $voucher_number ?></td></tr><?php endif; ?>
      <tr class="total-row"><th>Total Amount</th><td>LKR <?= $total ?></td></tr>
    </table>
