...
<div class="app-content" id="payment-app">
    <div class="container-fluid">
        ...
        <div class="card">
            ...
            <div v-if="checkGL && financeEnabled" class="je-preview mx-3 mt-3" :class="jeTotalAmount > 0 ? 'je-preview--active' : 'je-preview--empty'">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select" v-model="form.payment_type" :disabled="mode === 'view'">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" v-model="form.amount" step="0.01" min="0" :disabled="mode === 'view'" placeholder="0.00">
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Remark</label>
                                <input type="text" class="form-control" v-model="form.remark" :disabled="mode === 'view'" placeholder="Optional remark">
                            </div>
                        </div>
                    </div>
                </form>
...
<!-- Driver Picker Modal -->
<div class="modal fade" id="paymentDriverPickerModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table id="paymentDriverTable" class="table table-sm table-hover table-bordered w-100">
...
<div class="modal fade" id="paymentCleanerPickerModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                <table id="paymentCleanerTable" class="table table-sm table-hover table-bordered w-100">
                    <thead><tr><th>Ref</th><th>Name</th><th>Phone</th><th>Status</th></tr></thead>
                    <tbody></tbody>
                </table>
            </div>
...
<!-- Search Modal -->
<div class="modal fade" id="paymentSearchModal" tabindex="-1">
    <div class="modal-dialog modal-xl">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title"><i class="bi bi-search me-2"></i>Search Payments</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <div class="modal-body">
                <table id="paymentSearchTable" class="table table-sm table-hover table-bordered w-100">
                    <thead><tr><th>Ref</th><th>Date</th><th>Type</th><th>Recipient</th><th>Payment Type</th><th>Amount (LKR)</th><th>Remark</th></tr></thead>
                    <tbody></tbody>
                </table>
            </div>
        </div>
...
<script src="/modules/operations/staff_payroll/payment_salary_disburse/payment_salary_disburse.js"></script>

<script>
let paymentDriverDT = null;
document.getElementById('paymentDriverPickerModal').addEventListener('show.bs.modal', function () {
    if (paymentDriverDT) { paymentDriverDT.ajax.reload(); return; }
    paymentDriverDT = $('#paymentDriverTable').DataTable({
        ajax: { url: '/modules/operations/payment_salary_disburse/payment_salary_disburse_data.php?action=list_drivers', dataSrc: '' },
        columns: [
            { data: 'ref' },
            { data: 'name' },
    ...
    $('#paymentDriverTable tbody').on('click', 'tr', function () {
        const row = paymentDriverDT.row(this).data();
        window.dispatchEvent(new CustomEvent('payment-driver-selected', { detail: row }));
        bootstrap.Modal.getInstance(document.getElementById('paymentDriverPickerModal')).hide();
    });
});

let paymentCleanerDT = null;
document.getElementById('paymentCleanerPickerModal').addEventListener('show.bs.modal', function () {
    if (paymentCleanerDT) { paymentCleanerDT.ajax.reload(); return; }
    paymentCleanerDT = $('#paymentCleanerTable').DataTable({
        ajax: { url: '/modules/operations/payment_salary_disburse/payment_salary_disburse_data.php?action=list_cleaners', dataSrc: '' },
        columns: [
            { data: 'ref' },
            { data: 'name' },
            { data: 'phone', defaultContent: '' },
    ...
    $('#paymentCleanerTable tbody').on('click', 'tr', function () {
        const row = paymentCleanerDT.row(this).data();
        window.dispatchEvent(new CustomEvent('payment-cleaner-selected', { detail: row }));
        bootstrap.Modal.getInstance(document.getElementById('paymentCleanerPickerModal')).hide();
    });
});

let paymentSearchDT = null;
document.getElementById('paymentSearchModal').addEventListener('show.bs.modal', function () {
    if (paymentSearchDT) { paymentSearchDT.ajax.reload(); return; }
    paymentSearchDT = $('#paymentSearchTable').DataTable({
        ajax: { url: '/modules/operations/payment_salary_disburse/payment_salary_disburse_data.php?action=list', dataSrc: '' },
        columns: [
            { data: 'ref' },
            { data: 'date' },
    ...
    $('#paymentSearchTable tbody').on('click', 'tr', function () {
        const row = paymentSearchDT.row(this).data();
        window.dispatchEvent(new CustomEvent('payment-selected', { detail: row }));
        bootstrap.Modal.getInstance(document.getElementById('paymentSearchModal')).hide();
    });
});
</script>
