...
createApp({
  ...
  mounted() {
    axios.get('/modules/reports/staff_reports/cleaner_advance_report/cleaner_advance_report_data.php?action=list_cleaners')
  ...
  methods: {
    ...
    load() {
      ...
      axios.get('/modules/reports/staff_reports/cleaner_advance_report/cleaner_advance_report_data.php?' + params.toString())
        .then(r => { this.rows = r.data.rows; this.summary = r.data.summary; this.ran = true; })
        .catch(() => { this.error = 'Failed to load report.'; })
        .finally(() => { this.loading = false; });
    },
  },
}).mount('#cleaner-advance-app');
