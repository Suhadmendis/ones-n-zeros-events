...
createApp({
  ...
  methods: {
    ...
    load() {
      ...
      this.error = '';
      axios
        .get('/modules/reports/financial_reports/monthly_profit/monthly_profit_data.php?action=report&year=' + this.year)
        .then(r => {
          this.rows = r.data;
          this.ran  = true;
    ...
    renderChart() {
      ...
      const el = document.querySelector('#monthlyProfitChart');
      if (!el) return;
      this.chart = new ApexCharts(el, {
        chart: { type: 'line', height: 280, toolbar: { show: false } },
        series: [{ name: 'Profit', data: this.rows.map(r => r.profit) }],
