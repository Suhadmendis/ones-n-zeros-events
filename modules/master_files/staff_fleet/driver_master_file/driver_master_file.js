// driver_master_file.js — Driver module Vue app
...
createApp({
  ...
  mounted() {
    this.fetchRefNumber();
    document.addEventListener('driver-selected', (e) => this.loadDriver(e.detail));
    document.addEventListener('driver-employee-selected', (e) => {
  ...
  methods: {
    ...
    loadDriver(data) {
      ...
      this.form.license_number = data.license_number;
    ...
    onPrint() {
      ...
      axios.get('/modules/master_files/staff_fleet/driver_master_file/driver_master_file_data.php?action=exists&ref=' + encodeURIComponent(this.form.ref))
        .then(res => {
          if (!res.data.exists) {
          ...
          window.open('/modules/master_files/staff_fleet/driver_master_file/driver_master_file_print.php?' + params.toString(), '_blank');
        })
    ...
    onSave() {
      ...
      const proceed = (action) => {
        axios.post('/modules/master_files/staff_fleet/driver_master_file/driver_master_file_data.php?action=' + action, this.form)
          .then(() => {
            this.saved = true;
            this.onReset();
            this.fetchRefNumber();
          })
          .catch(err => { this.error = 'Failed to save driver.'; console.error(err); })
