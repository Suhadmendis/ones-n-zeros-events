// maintenance_type.js — generated by Generate Module
...
createApp({
  ...
  computed: {
    isDirty() {
      return (
        ...
        this.form.service_interval_km !== 0 ||
        this.form.service_interval_days !== 0
      );
    },
  },
  ...
  mounted() {
    this.fetchRefNumber();
    document.addEventListener('maintenance_type-selected', (e) => this.loadRecord(e.detail));
  },
  ...
  methods: {
    ...
    onPrint() {
      ...
      axios.get('/modules/master_files/vehicle/maintenance_type/maintenance_type_data.php?action=exists&ref=' + encodeURIComponent(this.form.ref))
        .then(res => {
          if (!res.data.exists) {
        ...
        service_interval_km: String(this.form.service_interval_km ?? ''),
        service_interval_days: String(this.form.service_interval_days ?? ''),
          });
          window.open('/modules/master_files/vehicle/maintenance_type/maintenance_type_print.php?' + params.toString(), '_blank');
        })
    ...
    onSave() {
      ...
      const proceed = (action) => {
        axios.post('/modules/master_files/vehicle/maintenance_type/maintenance_type_data.php?action=' + action, this.form)
          .then(() => {
            this.saved = true;
            this.onReset();
            this.fetchRefNumber();
    ...
    loadRecord(data) {
      ...
      this.form.service_interval_km = data.service_interval_km;
      this.form.service_interval_days = data.service_interval_days;
      this.isExisting = true;
      this.saved = false;
      this.error = '';
    },
  },
}).mount('#maintenance-type-app');