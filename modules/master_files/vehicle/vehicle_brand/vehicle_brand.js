// vehicle_brand.js — generated by Generate Module
...
createApp({
  ...
  mounted() {
    this.fetchRefNumber();
    document.addEventListener('vehicle_brand-selected', (e) => this.loadRecord(e.detail));
  },
  ...
  methods: {
    ...
    onPrint() {
      ...
      axios.get('/modules/master_files/vehicle/vehicle_brand/vehicle_brand_data.php?action=exists&ref=' + encodeURIComponent(this.form.ref))
        .then(res => {
          if (!res.data.exists) {
        ...
        country: String(this.form.country ?? ''),
          });
          window.open('/modules/master_files/vehicle/vehicle_brand/vehicle_brand_print.php?' + params.toString(), '_blank');
        })
    ...
    onSave() {
      ...
      const proceed = (action) => {
        axios.post('/modules/master_files/vehicle/vehicle_brand/vehicle_brand_data.php?action=' + action, this.form)
          .then(() => {
            this.saved = true;
            this.onReset();
            this.fetchRefNumber();
