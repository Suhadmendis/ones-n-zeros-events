// fuel_type.js — generated by Generate Module
...
createApp({
  ...
  mounted() {
    this.fetchRefNumber();
    document.addEventListener('fuel_type-selected', (e) => this.loadRecord(e.detail));
  },
  ...
  methods: {
    ...
    onPrint() {
      ...
      axios.get('/modules/master_files/vehicle/fuel_type/fuel_type_data.php?action=exists&ref=' + encodeURIComponent(this.form.ref))
        .then(res => {
          if (!res.data.exists) {
        ...
        name: String(this.form.name ?? ''),
          ...
          window.open('/modules/master_files/vehicle/fuel_type/fuel_type_print.php?' + params.toString(), '_blank');
        })
    ...
    onSave() {
      ...
      const proceed = (action) => {
        axios.post('/modules/master_files/vehicle/fuel_type/fuel_type_data.php?action=' + action, this.form)
          .then(() => {
            this.saved = true;
            this.onReset();
            this.fetchRefNumber();
