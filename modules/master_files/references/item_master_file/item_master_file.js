// item_master_file.js — Item module Vue app
...
createApp({
  ...
  mounted() {
    this.fetchRefNumber();
    document.addEventListener('item-selected', (e) => this.loadItem(e.detail));
  },
  ...
  methods: {
    ...
    onPrint() {
      ...
      axios.get('/modules/master_files/references/item_master_file/item_master_file_data.php?action=exists&ref=' + encodeURIComponent(this.form.ref))
        .then(res => {
          if (!res.data.exists) {
          ...
          window.open('/modules/master_files/references/item_master_file/item_master_file_print.php?' + params.toString(), '_blank');
        })
    ...
    onSave() {
      ...
      const proceed = (action) => {
        axios.post('/modules/master_files/references/item_master_file/item_master_file_data.php?action=' + action, this.form)
          .then(() => {
            this.saved = true;
            this.onReset();
            this.fetchRefNumber();
          })
          .catch(err => { this.error = 'Failed to save item.'; console.error(err); })
