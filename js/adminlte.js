...
(function (global, factory) {
    ...
    const SELECTOR_SEARCH_TOGGLE = '[data-lte-toggle="search"]';
    const SELECTOR_SEARCH_BLOCK = '.navbar-search-block';
    const CLASS_NAME_SEARCH_OPEN = 'navbar-search-open';
    class NavbarSearch {
        _block;
        constructor(block) {
            this._block = block;
        ...
        open() {
            this._block.classList.remove('d-none');
            this._block.classList.add(CLASS_NAME_SEARCH_OPEN);
            const input = this._block.querySelector('input[type="search"]');
            input?.focus();
        }
        close() {
            this._block.classList.remove(CLASS_NAME_SEARCH_OPEN);
            this._block.classList.add('d-none');
        }
        toggle() {
            if (this._block.classList.contains(CLASS_NAME_SEARCH_OPEN)) {
                this.close();
            }
            else {
                this.open();
    ...
    onDOMContentLoaded(() => {
        const block = document.querySelector(SELECTOR_SEARCH_BLOCK);
        if (!block) {
            return;
        }
        const search = new NavbarSearch(block);
        const toggles = document.querySelectorAll(SELECTOR_SEARCH_TOGGLE);
        toggles.forEach(toggle => {
            toggle.addEventListener('click', event => {
                event.preventDefault();
                search.toggle();
                toggle.setAttribute('aria-expanded', block.classList.contains(CLASS_NAME_SEARCH_OPEN) ? 'true' : 'false');
            });
        });
        document.addEventListener('keydown', event => {
            if (event.key === 'Escape' && block.classList.contains(CLASS_NAME_SEARCH_OPEN)) {
                search.close();
                toggles.forEach(toggle => toggle.setAttribute('aria-expanded', 'false'));
            }
        });
        document.addEventListener('click', event => {
            if (!block.classList.contains(CLASS_NAME_SEARCH_OPEN)) {
                return;
            }
            const target = event.target;
            if (target instanceof Node && !block.contains(target) && !target.closest(SELECTOR_SEARCH_TOGGLE)) {
                search.close();
                toggles.forEach(toggle => toggle.setAttribute('aria-expanded', 'false'));
            }
        });
